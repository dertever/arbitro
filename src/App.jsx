  ...
  if (profile?.status === 'pending') return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--navy)' }}>
      <div style={{ background: 'white', borderRadius: 16, padding: 40, maxWidth: 400, textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.6, marginBottom: 20 }}>Tu solicitud está siendo revisada por un administrador. Recibirás un email cuando esté activada.</p>
        <button className="btn btn-outline" onClick={() => supabase.auth.signOut().then(() => window.location.href = '/login')}>Cerrar sesión</button>
      </div>
    </div>
  )
